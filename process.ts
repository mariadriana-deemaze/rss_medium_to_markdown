...
async function fetchAndParseFeed(url: string): Promise<ParsedFeed> {
  ...
  const parsed: ParsedFeed = await parseStringPromise(feedData);
  return parsed;
}
 ...
 * @returns
 */
const generateFeedMarkdown = (template: string, entry: RSSItem) => {
 ...
 */
const generateOutput = (template: string, data: ItemData) => {
